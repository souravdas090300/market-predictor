...
from ..services import history, sentiment
from . import candles, model
...
def get_signal(symbol: str, use_news: bool = True, retrain: bool = False) -> dict:
    ...
    live = data.get_live_quote(symbol)
    row = latest.iloc[0]
    result = {
        "symbol": symbol,
        "name": info["name"],
        ...
        "note": (
            f"Direction over the next {config.HORIZON_DAYS} completed trading days. "
            "Live price is shown for context and is not used to retrain the model."
        ),
    }
    history.log_signal(symbol, result)
    return result
