...
def _fit_calibrator(raw_oos: pd.Series, y: pd.Series) -> LogisticRegression:
    ...
    Gradient boosting is over-confident on noisy data. This maps its raw output to the
    frequency it actually achieved on unseen data, so a model with no skill collapses
    towards the base rate instead of shouting 85%.
    """
    mask = raw_oos.notna()
...
def _metrics(p: pd.Series, y: pd.Series, fwd: pd.Series, horizon: int) -> dict:
    ...
    return {
        ...
        # What you'd get by always guessing the more common outcome.
        ...
        # Average return per signal following the model's call (overlapping windows, before costs).
