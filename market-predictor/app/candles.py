...
def detect(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Trend context, measured on the bar before the pattern.
...
def shape_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    f["cndl_body"] = ((p["c"] - p["o"]) / rng).fillna(0.0)          # signed, -1..1
...
def ohlc_tail(df: pd.DataFrame, bars: int = 60) -> list[dict]:
    t = df.tail(bars)
    return [
        {"d": ts.strftime("%Y-%m-%d"), "o": round(float(r.open), 4), "h": round(float(r.high), 4),
